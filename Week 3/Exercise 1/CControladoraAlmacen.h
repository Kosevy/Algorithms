#pragma once
#include "CVProveedores.h"
#include "CVProductos.h"
...
void Controladora::InsertarProducto()
{
	...
	do
	{
		cout << "Ingrese el anual del producto: "; cin >> ano;
	} while (ano < 1950 || ano > 2100);
	do
	{
	...
	Producto* producto = new Producto(idproducto, nombre, tipo, cantidad, categoriaproduc, ano, precio, proveedor);
	...
	do {
		...
		for (int i = 0; i < vectorproductos->getNumProductos(); i++)
		{
			...
			if (producto->getNombre() == nombre)
			{
				...
				do
				{
					cout << "Ingrese nuevo anual del producto: "; cin >> ano;
				} while (ano < 1950 || ano > 2100);
				do
				{
...
void Controladora::EliminarProductosVencidos()
{
	...
	for (int i = vectorproductos->getNumProductos() - 1; i >= 0; i--)
	{
		if (vectorproductos->obtenerDatosProducto(i)->getAno() < 2020)
		{
			vectorproductos->eliminarProducto(i);
			found = true;
		}
